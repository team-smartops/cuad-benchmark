...
def read_saved_data(input_dir,evaluate=False,output_examples=False):
    ...
    if output_examples:
        ...
        return get_balanced_dataset(all_features['datasets'])
...
def load_and_cache_examples(args, tokenizer, evaluate=False, output_examples=False):
    ...
    print("File Path:", features_file, datasets_file, examples_file)
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and ((evaluate and not args.predict_file) or (not evaluate and not args.train_file)):
            ...
            if evaluate:
                ...
                with open(
                    os.path.join(args.data_dir, args.predict_file), "r", encoding="utf-8"
                ) as reader:
                    input_data = json.load(reader)["data"]
                print(args.data_dir, args.predict_file)
                counter = 0
                for i in range(0, len(input_data), 32):
                    examples = processor._create_examples(input_data[i:i+32], 'train')
                    for i,j in enumerate(range(0,len(examples),100)):
                        sub_examples = examples[j:j+100]
                        ...
                        if args.local_rank in [-1, 0]:
                            logger.info("Saving features into cached files %s, %s, %s", os.path.join(features_file,'features_'+str(counter)),os.path.join(datasets_file,'datasets_'+str(counter)),os.path.join(examples_file,'examples_'+str(counter)))
                            torch.save({"features": features}, os.path.join(features_file,'features_'+str(counter)))
                            torch.save({"datasets": dataset}, os.path.join(datasets_file,'datasets_'+str(counter)))
                            torch.save({"examples": sub_examples}, os.path.join(examples_file,'examples_'+str(counter)))
                        counter += 1
            else:
                #examples = processor.get_train_examples(args.data_dir, filename=args.train_file)'''
                with open(
                    os.path.join(args.data_dir, args.train_file), "r", encoding="utf-8"
                ) as reader:
                    input_data = json.load(reader)["data"]
                print(args.data_dir, args.train_file)
                examples = squad._create_examples(input_data, 'train')
                for i,j in enumerate(range(0,len(examples),10000)):
                    sub_examples = examples[j:j+10000]
                    features, dataset = squad_convert_examples_to_features(
                        examples=sub_examples,
                        tokenizer=tokenizer,
                        max_seq_length=args.max_seq_length,
